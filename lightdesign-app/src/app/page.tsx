  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-12">
      ...
      <div className="mb-12 flex items-end justify-between">
        <div>
          <p className="mb-1 text-xs font-medium uppercase tracking-widest text-zinc-400">Product Image Studio</p>
          <h1 className="text-3xl font-semibold tracking-tight text-zinc-900">AI 产品主图生成</h1>
        ...
        <Link
          href="/create"
          className="inline-flex items-center gap-2 rounded-xl bg-amber-500 px-5 py-3 text-sm font-semibold text-white shadow-sm shadow-amber-200 transition hover:bg-amber-600 active:scale-[0.98]"
        >
      ...
      <div className="mb-5 flex items-center justify-between">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-zinc-400">最近任务</h2>
        {!isEmpty && loaded && (
      ...
      {!loaded && (
        <div className="space-y-2">
          {[1, 2, 3].map(i => (
            <div key={i} className="flex items-center gap-4 rounded-2xl bg-white px-5 py-4">
      ...
      {isEmpty && (
        <div className="flex flex-col items-center py-20 text-center">
          ...
          <p className="text-sm font-medium text-zinc-400">还没有生成任务</p>
          <p className="mt-1 text-xs text-zinc-300">点击右上角按钮开始你的第一张产品主图</p>
        </div>
      )}
        ...
        <div className="space-y-2">
          {tasks.slice(0, 20).map(t => {
            ...
            return (
              <div
                key={t.id}
                className="group flex items-center gap-4 rounded-2xl border border-zinc-200/60 bg-white px-5 py-4 transition hover:border-zinc-300 hover:shadow-sm"
              >
                {/* Thumb */}
                ...
                <div className="flex-shrink-0 opacity-0 transition group-hover:opacity-100">
                  {isExported && (
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-600 transition hover:bg-zinc-50 active:scale-[0.98]"
                    >
                  ...
                  {isDraft && (
                    <Link
                      href="/create"
                      className="inline-flex items-center gap-1.5 rounded-lg bg-zinc-100 px-3 py-1.5 text-xs font-medium text-zinc-600 transition hover:bg-zinc-200 active:scale-[0.98]"
                    >
                      继续编辑
                    </Link>
                  )}
