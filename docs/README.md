...
## 目录目的
本目录用于沉淀“AI 电商产品图与海报生成网站”的产品探索、流程设计、页面设计、生成链路、模板规则、测试验证和工程实现材料。

当前阶段已进入工程 MVP 验证。文档必须同时记录“目标基线”和“当前实现”，避免 PRD、线框、工程代码和验收口径漂移。所有可交付内容应能回连到根目录状态文件：
- `PLANS.md`：计划与阶段目标
...
| `engineering/` | 工程实现说明、接口草案、技术验证记录 | engineer-* |
...
6. 与当前实现不一致的早期文档必须标注“待更新”或“已被实现替代”，不得继续作为验收依据。
